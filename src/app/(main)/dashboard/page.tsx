...
export default async function DashboardPage({ searchParams }: PageProps) {
  ...
  const stats = await requireApiFetch<ShopDashboardStats>(`/reports/dashboard?${query.toString()}`);

  const maxBar = Math.max(...stats.revenueSeries.map((b) => b.value), 1);
  const maxBreakdown = Math.max(...stats.revenueByCategory.map((c) => c.revenue), 1);
    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <Card>
          <Card.Header>
            <div className="flex items-center justify-between">
              <Card.Description>รายได้ · {PERIOD_LABELS[stats.period]}</Card.Description>
        ...
        <Card>
          <Card.Header>
            <div className="flex items-center justify-between">
              <Card.Description>สมาชิกใหม่</Card.Description>
              <KpiIcon icon={Users} />
            </div>
            <Card.Title className="text-2xl">{stats.newMembersThisMonth.toLocaleString("th-TH")}</Card.Title>
          </Card.Header>
          <Card.Content>
            <p className="text-muted text-xs">เดือนนี้</p>
          </Card.Content>
        </Card>
        <Card>
          <Card.Header>
            <div className="flex items-center justify-between">
              <Card.Description>Point คงเหลือในระบบ</Card.Description>
              <KpiIcon icon={Coins} />
            </div>
            <Card.Title className="text-2xl">{stats.totalPointsInSystem.toLocaleString("th-TH")}</Card.Title>
          </Card.Header>
          <Card.Content>
            <p className="text-muted text-xs">ของสมาชิกทั้งร้าน</p>
          </Card.Content>
        </Card>
        <Card>
          <Card.Header>
            <div className="flex items-center justify-between">
              <Card.Description>บริการขายดีสุด</Card.Description>
              <KpiIcon icon={TrendingUp} />
            </div>
            <Card.Title className="truncate text-2xl">{stats.topServices[0]?.name ?? "—"}</Card.Title>
          </Card.Header>
          <Card.Content>
            <p className="text-muted text-xs">
              {stats.topServices[0] ? formatBaht(stats.topServices[0].totalRevenue) : "ยังไม่มีข้อมูล"}
            </p>
          </Card.Content>
        </Card>
      </div>
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <Card className="lg:col-span-2">
          <Card.Header>
            <Card.Title>รายได้ร้าน</Card.Title>
        ...
        <Card>
          <Card.Header>
            <Card.Title>รายได้ตามกลุ่มบริการ</Card.Title>
              ...
              stats.revenueByCategory.map((c) => (
                <div key={c.name}>
                  <div className="mb-1.5 flex items-center justify-between text-sm">
                    ...
                    <span className="text-muted text-xs">{formatBaht(c.revenue)}</span>
                  </div>
                  <div className="h-1.5 overflow-hidden rounded-full bg-default">
                    <div
                      className="h-full rounded-full bg-accent"
                      style={{ width: `${percentOf(c.revenue, maxBreakdown)}%` }}
                    />
                  </div>
                </div>
              ))
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Card>
          <Card.Header>
            ...
            <Card.Description>จัดอันดับตามจำนวนที่ขาย</Card.Description>
          </Card.Header>
          <Card.Content className="flex flex-col gap-4">
        ...
        <Card>
          <Card.Header>
            ...
            <Card.Description>ทีมงานในร้าน</Card.Description>
          </Card.Header>
              ...
              stats.staffSales.map((s, i) => (
                <div key={s.staffId} className="flex items-center gap-3 border-border border-b py-2.5 last:border-0">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-[11px] text-muted">{s.billCount} บิล</p>
                  </div>
              ...
              stats.newMembers.map((m) => (
                <div key={m.id} className="flex items-center gap-2.5">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="truncate text-[11px] text-muted">{m.phone}</p>
                  </div>
              ...
              stats.birthdays.map((b) => (
                <div key={`${b.name}-${b.date}`} className="flex items-center gap-2.5">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="truncate text-[11px] text-muted">{b.pointBalance.toLocaleString("th-TH")} point</p>
                  </div>
