...
import { createReward, type RewardInput, updateReward } from "./actions";
...
export function RewardFormDialog({ reward }: RewardFormDialogProps) {
  ...
  async function onSubmit(data: z.output<typeof rewardSchema>) {
    ...
    const result = reward ? await updateReward(reward.id, input) : await createReward(input);
    if (!result.success) {
          ...
          <Modal.Dialog>
            <form noValidate onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col gap-4">
              <Modal.Header>
                ...
                <p className="text-muted text-sm">ตั้งรางวัลหรือบริการที่สมาชิกแลกด้วย point ได้</p>
              </Modal.Header>
