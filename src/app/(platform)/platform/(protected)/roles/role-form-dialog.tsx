...
import { createPlatformRole, updatePlatformRole } from "./actions";
...
export function RoleFormDialog({ catalog, role }: RoleFormDialogProps) {
  ...
  async function onSubmit(data: z.infer<typeof roleSchema>) {
    const result = role ? await updatePlatformRole(role.id, data) : await createPlatformRole(data);
    if (!result.success) {
          ...
          <Modal.Dialog>
            <form noValidate onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col gap-4">
              <Modal.Header>
                ...
                <p className="text-muted text-sm">กำหนดชื่อและสิทธิ์การใช้งานของ platform admin ที่ได้รับ role นี้</p>
              </Modal.Header>
                ...
                <Controller
                  ...
                  render={({ field, fieldState }) => (
                    ...
                    >
                      ...
                      <Input placeholder="เช่น ผู้ดูแลฝ่ายสนับสนุน" />
